...
# Email Id ::: harshada.bhangale@gmail.csu.fullerton.edu
...
# url variable for KeyValueSessionStore using KV_URL app config
kv_url = app.config['KV_URL']
...
# sets the key value
    def set_key(self, key, value):
        dict = {key: value}
        res = requests.post(kv_url, json=dict)
        return res.json()

# gets the key
    def get_key(self, key):
        res = requests.get(kv_url + "/" + key)
        return res.json()[key]

# deletes a key
    def delete_key(self, key):
        res = requests.delete(kv_url + "/" + key)
        return res.json()
